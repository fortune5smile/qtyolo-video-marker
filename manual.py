    ...
    def action_save(self):
        try:
            if self.is_save_video:
                ...
                self.filename = os.path.join(my_documents, f"output_{int(time.time())}.mp4")
                ...
                if self.save_video is None:
                    ...
                    self.save_video.open(self.filename, fourcc, self.raw_video_fps, sz, True)
        except Exception as e:
            print(e)
    ...
    def action_export(self):
        try:
            ...
            if self.save_video is not None:
                ...
                self.statusbar.showMessage(f"导出成功:{self.filename}")
                self.btn_save.setText("开始录制")
                self.is_save_video = False
        except Exception as e:
            print(e)
